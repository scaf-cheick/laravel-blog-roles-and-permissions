            ...
            <div class="col s12 m12 l12">
                <div class="card">
                    <div class="card-content">
                        <span class="card-title">Add new post</span>
                        <form action="{{route('post.store')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="file-field input-field">
                                ...
                                <div class="file-path-wrapper">
                                    <input class="file-path validate" name="picture" id="picture" type="text" placeholder="Image de mise en avant * " required>
                            ...
                            <div class="input-field">
                                ...
                                <select name="category_id" required="">
                                    @foreach($categories as $key => $type)
                                        <option value="{{$type->id}}" {{ old('category_id') === $type->id ? 'selected' : '' }}>{{$type->title}}</option>
                                    @endforeach
                                </select> 
                            ...
                            <div class="input-field">
                                ...
                                <textarea id="body" name="content" class="materialize-textarea" required style="height: 100px">{{ old('content') == null ? '' : old('content') }}</textarea> 
                            ...
                            {{-- <div class="input-field">
                                <select name="status" required="" class="browser-default">
                                    ...
                                    <option value="true" {{ old('status') == 'true' ? 'selected' : '' }}>Activé</option>
                                    <option value="false" {{ old('status') == 'false' ? 'selected' : '' }}>Désactivé</option>
                                </select> 
